...
const loadablePlugin = (0, _helperPluginUtils.declare)((api, babelOptions) => {
  const {
  ...
  function isValidIdentifier(path, loadableImportSpecifier, lazyImportSpecifier) {
    // `loadable()`
    if (loadableImportSpecifier && path.get('callee').isIdentifier({
    ...
    return loadableImportSpecifier && path.get('callee').isMemberExpression() && path.get('callee.object').isIdentifier({
      name: loadableImportSpecifier
          ...
          programPath.traverse({
            ImportDefaultSpecifier(path) {
              if (!loadableImportSpecifier) {
                ...
                loadableImportSpecifier = parent.source.value == '@loadable/component' && local && local.name;
              }
            },
