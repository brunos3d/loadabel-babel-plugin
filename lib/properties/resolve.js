...
function resolveProperty({
   ...
   */
  {
    ...
    const isFederated = moduleFederation || Array.isArray(properties) && properties.some(prop => prop.isObjectProperty() && prop.get('key').isIdentifier({
      name: 'federated'
    }) && prop.get('value').isBooleanLiteral({
      value: true
    }));
